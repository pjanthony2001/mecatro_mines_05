...
{
  //Get the information from the Wire1, stores in lastBarRawValue
  scan();
...
uint8_t SensorBar::readByte(uint8_t registerAddress)
{
  ...
  while ((Wire1.available() < 1) && (timeout != 0))
    timeout--;

  if (timeout == 0)
    return 0;

  readValue = Wire1.read();
...
unsigned int SensorBar::readWord(uint8_t registerAddress)
{
  ...
  while ((Wire1.available() < 2) && (timeout != 0))
    timeout--;

  if (timeout == 0)
    return 0;

  msb = (Wire1.read() & 0x00FF) << 8;
  lsb = (Wire1.read() & 0x00FF);
  readValue = msb | lsb;
...
void SensorBar::readBytes(uint8_t firstRegisterAddress, uint8_t * destination, uint8_t length)
{
  ...
  Wire1.beginTransmission(deviceAddress);
  Wire1.write(firstRegisterAddress);
  Wire1.endTransmission();
